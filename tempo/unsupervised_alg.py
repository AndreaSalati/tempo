...
def run(
    ...
    mean_disp_init_coef=[
        -4,
        -0.2,
    ],  # ** mean / disp relationship learning parameters **
    ...
    **kwargs
    ...
    core_clock_genes = list(
        filter(lambda x: adata.var.loc[x]["prop"] > 0, core_clock_genes)
    )
    ...
    if use_nb:
        ...
        if est_mean_disp_relationship:
            log_mean_log_disp_coef = (
                estimate_mean_disp_relationship.estimate_mean_disp_relationship(
                    adata,
                    mean_disp_init_coef,
                    mean_disp_log10_prop_bin_marks,
                    mean_disp_max_num_genes_per_bin,
                    min_log_disp=-10,
                    max_log_disp=10,
                )
            )
        ...
        log_mean_log_disp_coef = np.array(
            [1e-1000]
        )  # i.e. use Poisson (mean equals the variance; if log_disp = 1e-1000, np.exp(log_disp) ~ 0)
    log_mean_log_disp_coef = torch.Tensor(log_mean_log_disp_coef)

    # ** write out **
    mean_disp_fit_coef_fileout = "%s/log_mean_log_disp_poly_coef_0.txt" % (
        mean_disp_param_folder_out
    )
    ...
    print(
        "--- RESTRICTING CANDIDATE HIGHLY VARIABLE GENES BASED ON MINIMUM PROPORTION IN PSEUDOBULK ---"
    )
    adata = adata[
        :,
        (adata.var["prop"] >= min_gene_prop)
        | (np.isin(adata.var_names, core_clock_genes)),
    ]
    ...
    hv_genes, pearson_residuals, log1p_prop_mean, log1p_prop_var, hv_gene_indices = (
        hvg_selection.get_hv_genes_kernel(
            adata,
            std_residual_threshold=hv_std_residual_threshold,
            viz=False,
            bw=0.05,
            pseudocount=1,
        )
    )  # bw = 0.1
    hv_genes = np.setdiff1d(
        np.array(hv_genes), np.array(core_clock_genes)
    )  # make sure the core clock genes are not in there
    ...
    adata = adata[
        :,
        (np.isin(adata.var_names, hv_genes))
        | (np.isin(adata.var_names, core_clock_genes)),
    ]
    ...
    core_clock_genes = list(
        clock_adata[
            :, clock_adata.var["var_pearson_residual"] >= clock_std_residual_threshold
        ].var_names
    )
    ...
    # ** run **
    null_log_evidence_vec = generate_null_dist.run(
        clock_adata.copy(),
        evidence_folder_out,
        log_mean_log_disp_coef,
    ...
    while True:
        ...
        if algorithm_step >= 1:
            previous_alg_step_subfolder = "%s/%s" % (
                alg_result_head_folder,
                algorithm_step - 1,
            )
            cycler_adata = (
                utils.init_cycler_adata_variational_and_prior_dist_from_prev_round(
                    cycler_adata,
                    previous_alg_step_subfolder,
                    enforce_de_novo_cycler_flat_Q_prior=False,
                )
            )

        # ** estimate cell phase from clock + de novo cyclers **
        phase_est_folder_out = alg_step_subfolder
        opt_cycler_theta_posterior_likelihood, opt_cycler_gene_param_dict_unprepped = (
            est_cell_phase_from_current_cyclers.run(
                cycler_adata,
                phase_est_folder_out,
                log_mean_log_disp_coef,
                **config_dict
            )
        )
        ...
        # ** compute **
        clock_log_evidence = compute_clock_evidence.run(
            cycler_adata,
            opt_cycler_gene_param_dict_unprepped,
            opt_cycler_theta_posterior_likelihood,
            log_mean_log_disp_coef,
            **config_dict
        )

        # ** write out evidence **
        fileout = "%s/step_%s_clock_evidence.txt" % (
            evidence_folder_out,
            algorithm_step,
        )
        ...
        log10_bf_tempo_vs_null = (
            clock_log_evidence - np.max(null_log_evidence_vec)
        ) / np.log(10)

        # ** halt algorithm progression if not sufficiently better than random **
        print(
            "Tempo vs. null clock evidence log10 bayes factor: %s"
            % log10_bf_tempo_vs_null
        )
        if log10_bf_tempo_vs_null < log10_bf_tempo_vs_null_threshold:
            print(
                "Clock evidence not sufficiently better than random. Halting algorithm."
            )
            break
        ...
        if (
            algorithm_step == 0
            and log10_bf_tempo_vs_null >= log10_bf_tempo_vs_null_threshold
        ):
        ...
        if algorithm_step == 0:
            ...
            print(
                "Clock log evidence decreased from previous Tempo step. Halting algorithm."
            )
        ...
        if algorithm_step >= 1:
            ...
            hv_adata = utils.init_hv_adata_variational_and_prior_dist_from_prev_round(
                hv_adata, previous_alg_step_subfolder
            )

        # ** id de novo cyclers **
        de_novo_cycler_detection_folder_out = alg_step_subfolder
        new_de_novo_cycler_genes = identify_de_novo_cyclers.run(
            hv_adata,
            opt_cycler_theta_posterior_likelihood,
            de_novo_cycler_detection_folder_out,
            log_mean_log_disp_coef,
            **config_dict
        )
    ...
    if better_than_random:
        ...
        opt_cycler_gene_df_path_out = (
            "%s/cycler_gene_prior_and_posterior.tsv" % opt_folder_out
        )
        opt_flat_gene_df_path_out = (
            "%s/flat_gene_prior_and_posterior.tsv" % opt_folder_out
        )

        # read in cell phase data and write out
        opt_cell_posterior_df_path = (
            "%s/%s/cell_phase_estimation/cell_posterior.tsv"
            % (alg_result_head_folder, alg_step_to_return)
        )
        opt_cell_posterior_df = pd.read_table(
            opt_cell_posterior_df_path, sep="\t", index_col="barcode"
        )
        opt_cell_posterior_df.to_csv(opt_cell_posterior_df_path_out, sep="\t")

        # read in cycler gene data and write
        opt_cycler_gene_df_path = (
            "%s/%s/cell_phase_estimation/gene_prior_and_posterior.tsv"
            % (alg_result_head_folder, alg_step_to_return)
        )
        opt_cycler_gene_df = pd.read_table(
            opt_cycler_gene_df_path, sep="\t", index_col="gene"
        )
        ...
        if use_de_novo_cycler_detection:
            ...
            opt_flat_gene_df_path = (
                "%s/%s/de_novo_cycler_id/gene_prior_and_posterior.tsv"
                % (alg_result_head_folder, alg_step_to_return)
            )
            try:
                opt_flat_gene_df = pd.read_table(
                    opt_flat_gene_df_path, sep="\t", index_col="gene"
                )

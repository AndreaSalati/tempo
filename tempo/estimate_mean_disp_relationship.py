...
def estimate_mean_disp_relationship(
    adata,
    init_coef,
    log10_prop_bin_marks,
    max_num_genes_per_bin,
    min_log_disp=-10,
    max_log_disp=10,
):
    """
    ...
    init_coef: list
        The initial coefficients for the polynomial fit.
    log10_prop_bin_marks: list
        The bin marks for log10 proportion.
    max_num_genes_per_bin: int
        The maximum number of genes to sample per bin.
    min_log_disp: float
        The minimum log dispersion value to consider.
    max_log_disp: float
        The maximum log dispersion value to consider.
    """
    ...
    for bin_index in range(0, len(log10_prop_bin_marks) - 1):
        bin_start_log10_prop, bin_end_log10_prop = (
            log10_prop_bin_marks[bin_index],
            log10_prop_bin_marks[bin_index + 1],
        )
        bin_gene_dict[bin_index] = np.array(
            adata.var[
                (adata.var["log10_prop"] >= bin_start_log10_prop)
                & (adata.var["log10_prop"] < bin_end_log10_prop)
            ].index
        )
    ...
    for bin_index in range(0, len(log10_prop_bin_marks) - 1):
        if bin_gene_dict[bin_index].size > max_num_genes_per_bin:
            bin_gene_dict[bin_index] = np.random.choice(
                bin_gene_dict[bin_index], size=max_num_genes_per_bin, replace=False
            )
    ...
    for gene_index in tqdm.tqdm(range(0, subsampled_adata.shape[1])):
        ...
        mean_model = smf.glm(
            ...
            offset=np.log(np.array(subsampled_adata.obs["lib_size"])),
        )
    ...
    def estimate_gene_nb_coefs(
        gene_lambda,
        gene_X,
        log_L,
        log_L_coef,
        log_disp_guess=-1,
        fixed_mean_lib_size_coef=True,
    ):
        ...
        if fixed_mean_lib_size_coef:  # BFGS
            res = scipy.optimize.minimize(
                compute_gene_nb_nll_fixed_mean_and_lib_coef,
                [log_disp_guess],
                args=(gene_X, log_L, gene_lambda, log_L_coef),
                method=method,
            )
        # else:
        #     res = scipy.optimize.minimize(
        #         compute_gene_nb_nll_learnable_mean_and_lib_coef,
        #         [log_disp_guess, gene_lambda, log_L_coef],
        #         args=(gene_X, log_L),
        #         method=method,
        #     )
    ...
    def estimate_nb_coefs(adata, gene_lambda_vec, log_L_coef, fixed_mean_lib_size_coef):
        ...
        for gene_index in tqdm.tqdm(range(0, adata.shape[1])):
            ...
            # run
            param_est_dict, hessian_inverse, nll = estimate_gene_nb_coefs(
                gene_lambda,
                gene_X,
                log_L,
                log_L_coef[gene_index],
                fixed_mean_lib_size_coef=fixed_mean_lib_size_coef,
            )
    ...
    # estimate dispersion parameters with scipy minimize and nll
    subsampled_gene_nb_param_est_df = estimate_nb_coefs(
        subsampled_adata,
        gene_lambda_vec=subsampled_gene_mu,
        log_L_coef=np.ones((adata.shape[1])),
        fixed_mean_lib_size_coef=True,
    )
    ...
    def poly_mse(params, *args):
        ...
        log_disp_hat = np.matmul(
            np.power(mu.reshape(-1, 1), np.arange(0, poly_power + 1)), params
        )  # [num_genes]
    ...
    def estimate_poly(log_disp, mu, init_coef):
        ...
        # run optimization
        res = scipy.optimize.minimize(
            poly_mse, init_coef, args=(mu, log_disp), method="Powell"
        )

        return res

    # get rid of the outlier dispersion estimates
    subsampled_gene_nb_param_est_df_no_outliers = subsampled_gene_nb_param_est_df[
        (subsampled_gene_nb_param_est_df["log_disp"] >= min_log_disp)
        & (subsampled_gene_nb_param_est_df["log_disp"] <= max_log_disp)
    ]

    # fit the polynomial
    mean_disp_fit_res = estimate_poly(
        log_disp=np.array(subsampled_gene_nb_param_est_df_no_outliers["log_disp"]),
        mu=np.array(subsampled_gene_nb_param_est_df_no_outliers["mu"]),
        init_coef=init_coef,
    )
